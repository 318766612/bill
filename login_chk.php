        ...
        if (empty($user_name) || empty($user_email) || empty($user_pass)) {
            ...
            if ($row = $results->fetchArray()) {
                ...
                if ($query) {
                    ...
                    $sqlBank = "insert into " . TABLE . "bank (bankname, bankaccount, balancemoney,addtime,updatetime,userid) values ('默认账户','','','.$addtime.','" . $addtime . "','" . $uid . "')";
                    $queryBank = $conn->exec($sqlBank);
                    if ($queryBank) {
                        $error_code = $error_code . "增加默认账户成功！";
                    } else {
                        $error_code = $error_code . "增加默认账户出错！";
                    }
                } else {
