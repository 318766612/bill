...
if ($getaction == "addrecord") {
    ...
    if (empty($classid) or empty($money) or empty($bankid)) {
        $error_code = "缺少参数！请检查钱数、分类、账户是否存在";
...
if ($getaction == "saverecord") {
    ...
    if (empty($id) or empty($money)) {
        ...
        if ($result) {
            if ($zhifu == 2) {
                if ($old_bankid <> $new_bankid && $old_bankid > 0) {
                    money_int_out($old_bankid, $money, "1");
                }
                if ($old_bankid <> $new_bankid && $new_bankid > 0) {
                    money_int_out($new_bankid, $money, "2");
                }
            } else {
                if ($old_bankid <> $new_bankid && $old_bankid > 0) {
                    money_int_out($old_bankid, $money, "2");
                }
                if ($old_bankid <> $new_bankid && $new_bankid > 0) {
                    money_int_out($new_bankid, $money, "1");
                }
            }
            $success = "1";
            $error_code = "保存成功！";
...
if ($getaction == 'import') {
    ...
    if ($len_result <= 1) {
        alertgourl("你的文件没有任何数据！", "int_out.php");
    }
    $data_values = "";
    $insert_count = 0;
